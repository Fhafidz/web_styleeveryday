...
const createNewUser = async (req, res) => {
    ...
    try {
        ...
        const newUser = {
            ...
            useraccount_id: uuidv4() // Generate a unique ID
        };
...
const loginUser = async (req, res) => {
    ...
    if (!email || !password) {
        return res.status(400).json({
            error: "Please provide both email and password",  // Updated error message
        });
    }

    try {
        const [user] = await database.query('SELECT useraccount_id, password FROM useraccount WHERE email = ?', [email]);
        if (user.length > 0) {
            const passwordMatch = await bcrypt.compare(password, user[0].password);
            if (passwordMatch) {
                const token = jwt.sign({ useraccount_id: user[0].useraccount_id }, process.env.JWT_SECRET);
                res.json({
                    token,
                    user: {
                        useraccount_id: user[0].useraccount_id
                    }
                });
            } else {
                res.status(401).json({
