...
const listUserFavorites = async (req, res) => {
  const useraccount_id = req.params.useraccount_id; // Use the correct field name
  try {
    const [results] = await database.query(
      `SELECT * FROM user_favorites JOIN item ON user_favorites.item_id = item.item_id WHERE user_favorites.useraccount_id = ?`, 
      [useraccount_id]
    );
